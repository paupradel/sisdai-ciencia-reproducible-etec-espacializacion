...
Este ejemplo es para agregar información geográfica a los datos del ["Secretariado Ejecutivo del Sistema Nacional de Seguridad Pública"](https://www.gob.mx/sesnsp). En particular, vamos a utilizar los datos de carpetas de investigación del fueron común abiertas a nivel municipal que se pueden encontrar [aquí](https://www.gob.mx/sesnsp/acciones-y-programas/incidencia-delictiva-del-fuero-comun-nueva-metodologia?state=published). Desde este sitio se puede descargar una carpeta con todos los datos acumulados desde el año 2015 hasta la última fecha de actualización. Para correr este código, por favor descarga el archivo zip y estrae los archivos para poder utilizar el archivo `*.csv`.  

Adicionalmente, para la información geográfica respecto a estados y municipios, vamos a utilizar capas geográficas en formato geopaquete descargadas desde el [portal de Gema](gema.conahcyt.mx).
...
secretariado_municipal <- 
    read.csv("datos/datos-originales/Municipal-Delitos-2015-2024_ago2024/Municipal-Delitos-2015-2024_ago2024.csv",
    encoding = "latin1")

# Capas con informacion geografica
#estatal <- st_read("datos/datos-auxiliares/gref_division_estatal_20_est_a.gpkg")
municipal <- st_read("datos/datos-auxiliares/gref_division_municipal_23_mun_a.gpkg")
...
Podemos ver que los datos vienen organizados de forma que se indica cuántas carpetas de investigación se abrieron en cada municipio, en cada año, por cada uno de los meses. Es importante tomar en cuenta que este conjunto de datos se actualiza mensualmente, por lo que al momento en que estamos generando este ejemplo (octubre 2024) no contamos con datos completos del año en curso, por lo que no los vamos a incluir.

En este ejemplo vamos a utilizar únicamente información de carpetas de investigación abiertas por feminicidio y agruparemos la información por año. Compo parte del proceso de transformación, es importante revisar la completitud y calidad de los datos. 
